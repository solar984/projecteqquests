...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13898})) then -- bag of ice necklaces
		...
		e.other:Faction(233,15); --faction with Crimson hands increased
		e.other:Faction(266,1); --faction with High Council of Erudin increased
		e.other:Faction(265,-2); --faction with Heretics decreased
		e.other:Faction(267,2); --faction with High Guards of Erudin increased
		e.other:QuestReward(e.self,0,7,0,0,12208,1000);
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 12207})) then -- Half of a Spell
		...
		-- Confirmed Live Factions
		e.other:Faction(233,5); --faction with Crimson hands increased
		e.other:Faction(266,1); --faction with High Council of Erudin increased
		e.other:Faction(265,-1); --faction with Heretics decreased
		e.other:Faction(267,1); --faction with High Guards of Erudin increased
