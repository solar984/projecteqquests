...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 1598})) then -- Black Stone Candlestick
		...
		e.other:QuestReward(e.self,0,0,0,0,1596); -- Magical Suspension Fluid
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 1597})) then -- Divinatory Concoction 
		...
		e.other:Faction(246,15,0); -- Faction: Faydarks Champions
		e.other:Faction(275,3,0); -- Faction: Keepers of the Art
		e.other:Faction(279,2,0); -- Faction: King Tearis Thex
		e.other:Faction(239,-3,0); -- Faction: The Dead
		e.other:QuestReward(e.self,0,0,0,0,1599,1500); -- Powder of Unanimation
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 1056})) then --Faded Cloak
		...
		e.other:Faction(279,2,0); -- Faction: King Tearis Thex
		e.other:Faction(239,-3,0); -- Faction: The Dead
		e.other:QuestReward(e.self,0,0,0,0,1057); --Mystic Cloak
	end
