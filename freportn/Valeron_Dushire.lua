...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 18737})) then --Tattered Note
		...
		e.other:QuestReward(e.self,{itemid = 13554, exp = 20}); --Faded Purple Tunic*
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 6701})) then --a dusty old leg bone
		...
		e.other:QuestReward(e.self,{itemid = 6701, exp = 1}); -- Item: A Dusty Old Leg Bone
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 18827})) then --a testimony of truth
		...
		e.other:Faction(362,20,0); --Priests of Marr
		e.other:Faction(311,10,0); --Steel Warriors
		e.other:QuestReward(e.self,{itemid = 13947, exp = 5000}); -- Brilliant Sword of Faith
	end
