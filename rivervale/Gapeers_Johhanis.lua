...
function event_say(e) 
	if(eq.is_the_scars_of_velious_enabled()) then
		...
		elseif(e.message:findi("what specimens")) then
			if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 50) then -- 0 indiff isn't high enough, amiable is, need to find exact number for all these lines
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 1736, item2 = 1737, item3 = 1735},1,text)) then
		...
		e.other:Faction(300,5); -- Faction: Priests of Mischief
