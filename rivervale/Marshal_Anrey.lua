...
function event_say(e)
	...
	elseif(e.message:findi("petitioning")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 200) then -- requires amiably	(200)	
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 200 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13761,item2 = 13756,item3 = 13075,item4 = 13749})) then -- requires amiably, Polar Bear Skin, Thick Grizzly Bear Skin, Shark Skin, Alligator Skin
		...
		e.other:Faction(263,25); -- Faction: Guardians of the Vale
		e.other:Faction(286,3); -- Faction: Mayor Gubbin
		e.other:Faction(355,2); -- Faction: Storm Reapers
		e.other:Faction(292,2); -- Faction: Merchants of Rivervale
		e.other:Faction(334,-3); -- Faction: Dreadguard Outer
		e.other:QuestReward(e.self,0,0,math.random(10),1,13941,1000); -- Leatherfoot Skullcap
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 200 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13941,item2 = 13942})) then -- Leatherfoot Skullcap, Dragoon Dirk
		...
		-- verified live faction
		e.other:Faction(263,50); -- Faction: Guardians of the Vale
		e.other:Faction(286,7); -- Faction: Mayor Gubbin
		e.other:Faction(355,5); -- Faction: Storm Reapers
		e.other:Faction(292,5); -- Faction: Merchants of Rivervale
		e.other:Faction(334,-7); -- Faction: Dreadguard Outer
