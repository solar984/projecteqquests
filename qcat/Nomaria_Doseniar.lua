function event_say(e)
	if(e.message:findi("hail")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0) then -- need exact faction, only tested 0 (indiff) and scowls
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 18807})) then -- Sealed Letter [Note To Jaggedpine - real]
		...
		e.other:Faction(221,50); -- Bloodsabers
		e.other:Faction(262,-7); -- Guards of Qeynos
		e.other:Faction(296,5); -- Opal Dark Briar
		e.other:Faction(341,-12); -- Priest of Life
		e.other:Faction(230,2); -- Corrupt Qeynos Guards
