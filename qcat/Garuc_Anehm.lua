function event_say(e)
	if(e.message:findi("Hail")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0) then -- not sure of exact faction, responds between 0 and Scowls
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13134})) then -- Hurrietas Bloody Dress
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13908})) then -- Busted Prayer Beads (*Trintles Prayer Beads)
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13296})) then -- Prayer Beads (*Temple of Life Beads)
	...
	-- Requires Amiably or better Faction
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13287})) then -- Order of Thunder (*Order Of Thunder Medal)
		e.self:Say("Fine work! One less threat to our shrine. Take this. It shall help you become a greater asset to our shrine. Go forth and spread the disease.");
		-- Confirmed Live Factions		
		e.other:Faction(221,10);  -- Faction: Bloodsabers
		e.other:Faction(262,-1); -- Faction: Guards of Qeynos
