...
function event_trade(e)
	...
	local qglobals = eq.get_qglobals(e.other);
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 13225})) then
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 13226})) then
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 18840})) then
		...
		e.other:Faction(5001,3,0); -- Anti Mage
		e.other:AddEXP(500);
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 12330,item2 = 12329,item3 = 13227})) then -- A Large Locked Crate, Blue Orc Head, Black Heart
		...
		e.other:Faction(5001,3,0); -- Anti Mage
		e.other:AddEXP(500);
	elseif(qglobals["paladin_epic"] == "9" and item_lib.check_turn_in(e.self, e.trade, {platinum = 1000})) then
	...
	elseif(qglobals["paladin_epic"] == "9" and item_lib.check_turn_in(e.self, e.trade, {platinum = 500})) then
