function event_say(e)
	...
	elseif(e.message:findi("pickup.* reagent")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) > 5) then -- apprehensively or worse
		...
		elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) > 4) then -- indifferently
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 12125})) then -- Shark Powder
		...
		e.other:QuestReward(e.self,{gold = math.random(30), itemid = 15016, exp = 100000}); -- spell: smite
