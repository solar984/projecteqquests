function event_say(e)
	...
	elseif(e.message:findi("Lomarc")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 50) then -- mid indifferent to start
	...
	elseif(e.message:findi("him.* mermaid")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 50) then
			...
			e.other:SummonItem(18793); -- Item: Lomarc's Payment
		elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0) then
			e.self:Say("The Circle has recognized your deeds and contributions, but I think you need to prove your worth to us a little more.");
		else
			e.self:Say("Heh...  With all you've done, I'm surprised you're still alive.");
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 40 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13716})) then -- Kerran Doll
		...
		e.other:Faction(223,30,0);		-- Circle of Unseen Hands
		...
		e.other:Faction(273,4,0);		-- Kane Bayle
