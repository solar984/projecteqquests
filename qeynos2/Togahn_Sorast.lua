...
function event_say(e)
	...
	elseif(e.message:findi("white") or e.message:findi("yellow") or e.message:findi("orange")) then
		e.self:Say("Brother Phin gives those headbands out. Go see him about earning one.");
	...
	elseif(e.message:findi("red headband")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 275) then
			e.self:Say("Ah, you think you are skilled enough to earn the red headband of the Silent Fist Clan, " .. e.other:GetCleanName() .. "? Hmmm.. Well, if you can help us find [Brother Dareb], you would certainly deserve the honor of wearing the red headband.");
	...
	elseif(e.message:findi("evil items")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 350) then
			e.self:Say("These evil creations have been found all over Antonica, and no one knows, or at least no one admits to knowing, where exactly they were found, but we do know that they are currently in the wrong hands and must be destroyed. Please, " .. e.other:GetCleanName() .. ", seek out the [Skull of Jhen'Tra], the [Dagger of Marnek], and the [Coronet of Buried Blood], and bring them to me.");
		elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0) then
			e.self:Say("I have been watching you, and appreciate the help you've given to the brothers and sisters of the Silent Fist. But, I feel that such a vital matter should be left to one of our more trusted members.");
		else
			e.self:Say("Your previous actions disgust me. Our guild will have nothing to do with the likes of you. Now, please, leave us be!");
...
function event_trade(e)
	...
	local text2 = "Good work, " .. e.other:GetCleanName() .. ", your task is almost completed.";

	-- Orange Headband, Head of Ghanex Drah, Dareb's Skull, Head of Shen
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 275 and item_lib.check_turn_in(e.self, e.trade, {item1 = 10112, item2 =13165, item3 = 13166,item4 = 13167},1,text1)) then
		e.self:Say("We are deeply honored to have such great warrior as part of our Clan. With a proper burial, Shen and Dareb's souls will finally be at peace, and with the death of that vile Ghanex, the plains will be much safer for travelers. It is a great honor to present you, " .. e.other:GetCleanName() .. ", with the red headband of the Silent Fist Clan.");
		e.other:Faction(309,75); -- Silent Fist Clan
		e.other:Faction(262,11); -- Guards of Qeynos
		e.other:Faction(361,5); -- Ashen Order
		e.other:QuestReward(e.self,{itemid = 10113, exp = 1600}); -- Item: Red Headband
	-- Red Headband, Skull of Jhen`Tra, Dagger of Marnek, Zaharn's Coronet
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 350 and item_lib.check_turn_in(e.self, e.trade, {item1 = 10113, item2 =13168, item3 = 13169,item4 = 3000},1,text2)) then
		e.self:Say("In honor of your recent acts of courage and dedication, I reward you, " .. e.other:GetCleanName() .. ", with the purple headband of the Silent Fist Clan.");
		e.other:Faction(309,100); -- Silent Fist Clan
		e.other:Faction(262,15); -- Guards of Qeynos
		e.other:Faction(361,5); -- Ashen Order
		e.other:QuestReward(e.self,{itemid = 10114, exp = 3200}); -- Purple Headband
	end
