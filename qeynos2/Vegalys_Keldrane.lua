...
function event_say(e)
	...
	elseif(e.message:findi("investigator")) then
		e.self:Say("Very well then, " .. e.other:GetCleanName() .. ". We will not allow just anyone to wear the official Investigator's Badge and please forgive me. . . But it is hard to tell where one's loyalties lie these days. If you are truly [serious] then there is a duty you must perform first to show you are on the side of Antonius Bayle, the council and the good people Qeynos.");
	...
	elseif(e.message:findi("advance") and e.other:HasItem(18289)) then
	...
	elseif(e.message:findi("advance") and e.other:HasItem(2386)) then
	...
	elseif(e.message:findi("advance") and e.other:HasItem(2583)) then
	...
	elseif(e.message:findi("interrogators")) then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 2173},0)) then -- Cracked Corrupt Guard Helm
		...
		e.other:QuestReward(e.self,{itemid = 18289, exp = 2000});
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 2386},0)) then -- Investigator's Badge
		...
		e.other:QuestReward(e.self,{itemid = 18292, exp = 2000});
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 2583},0)) then -- Researcher's Badge
		...
		e.other:QuestReward(e.self,{itemid = 2694, exp = 2000}); -- Seal of Vegalys
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 2694,item2 = 2392})) then -- Seal of Vegalys and Head of Azibelle Spavin
		...
		e.other:QuestReward(e.self,{itemid = 2388, exp = 2000}); -- Qeynos Badge of Honor
	end
