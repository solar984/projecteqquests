...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13306})) then -- must be amiable
		e.self:Say("Oh, turning in your key, are you? Very well, defender of life. Here you are.");
		e.other:QuestReward(e.self,{itemid = eq.ChooseRandom(15126,15015,15248)}); -- Item(s): Spell: Inspire Fear (15126), Spell: Greater Healing (15015), Spell: Ward Summoned (15248)
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13296, gold=100},1,text)) then 
		e.self:Say("I see your beads need to be charged. Very well, here you are. Use them and good health to you!!!");
		e.other:QuestReward(e.self,{itemid = 13296}); -- Item: Prayer Beads
	end
