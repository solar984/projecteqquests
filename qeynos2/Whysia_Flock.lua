...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 6356, gold=20})) then -- Shining Star of Light
		...
		e.other:QuestReward(e.self,{itemid = 6356}); -- Note this quest is superficial because the shining star has no charges
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13296, gold=100},1,text)) then -- must be amiable
