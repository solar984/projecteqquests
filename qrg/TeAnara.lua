...
function event_enter(e)
	if e.other:HasItem(18713) then
		e.other:Message(MT.Yellow,"As you orient yourself amongst the peacefulness of the Surefall Glade, a melodic, peaceful voice seems to float on the wind. 'I am Te'Anara, Master of the Woods. Read the note in your inventory and when you wish to begin your training, hand it to me. We must preserve the heart of the Jaggedpine lest all of Norrath will perish!'");
	end
end
...
function event_say(e)
	...
	elseif(e.message:findi("special potion")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 50) then -- mid indifferent
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 18713})) then -- Tattered Note
		...
		e.other:Faction(272, 100); --Jaggedpine Treefolk
		e.other:Faction(302, 25); --Protectors of Pine
		e.other:Faction(343, 15); --QRG Protected Animals
		e.other:Faction(324, -25); --Unkempt Druids
		e.other:Faction(262, 15);   --Guards of Qeynos
		e.other:QuestReward(e.self,{itemid = 13510, exp = 20}); -- Item: Dried Grass Tunic*
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 18809})) then
		...
		e.other:Faction(302, 2,0); --Protectors of Pine
		e.other:Faction(343, 1,0); --QRG Protected Animals
		e.other:Faction(324, -2,0); --Unkempt Druids
		e.other:Faction(262, 1,0);   --Guards of Qeynos
		e.other:QuestReward(e.self,{itemid = 15222, exp = 100}); -- Item: Spell: Invigor
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 18807})) then -- Sealed Letter [Note To Jaggedpine - real]
		...
		e.other:Faction(272, 30,0); --Jaggedpine Treefolk
		e.other:Faction(302, 7,0); --Protectors of Pine
		e.other:Faction(343, 4,0); --QRG Protected Animals
		e.other:Faction(324, -7,0); --Unkempt Druids
		e.other:Faction(262, 4,0);   --Guards of Qeynos
		e.other:QuestReward(e.self,{silver = math.random(10), gold = math.random(5), exp = 5000});
