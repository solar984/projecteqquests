function event_say(e)
	...
	elseif(e.message:findi("join the hunt")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0) then -- verified faction between -100 and 0
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 13813})) then -- accepts at kos even though dialogue for quest rejects
		...
		e.other:Faction(262,6); -- Guards of Qeynos
		e.other:QuestReward(e.self,{silver = 2, itemid = 8803, exp = 6000}); -- Rough Elm Recurve Bow
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 13825})) then -- accepts at kos even though dialogue for quest rejects
		...
		e.other:Faction(302,15); -- Protectors of Pine
		e.other:Faction(272,3); -- Jaggedpine Treefolk
		e.other:Faction(306,-2); -- Sabertooths of Blackburrow
		e.other:Faction(262,3); -- Guards of Qeynos
		e.other:QuestReward(e.self,{silver = 2, exp = 6000});
	end
