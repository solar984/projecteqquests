...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= -500 and item_lib.check_turn_in(e.self, e.trade, {item1 = 18911})) then
		...
		e.other:Faction(272,15); -- Jaggedpine Treefolk
		e.other:Faction(302,3); -- Protector of the Pine
		e.other:Faction(343,2); -- QRG Protected Animals
		e.other:Faction(324,-3); -- Unkempt Druids
		e.other:Faction(262,2); -- Guards of Qeynos
		e.other:QuestReward(e.self,0,math.random(10),math.random(10),0,18912,1000);
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= -500 and item_lib.check_turn_in(e.self, e.trade, {item1 = 12141})) then -- hand in worked at apprehensive, contrary to quest text being rejected at indifferent, didn't work at threatening so it's between -100 and -500
		...
		e.other:Faction(302,20); -- confirmed live factions
		e.other:Faction(272,5); -- Faction: Jaggedpine Treefolk
		e.other:Faction(343,3); -- Faction: QRG Protected Animals
		e.other:Faction(324,-5); -- Faction: Unkempt Druids
