...
function event_say(e)
	...
	elseif(e.message:findi("brave fighter")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100) then
			...
			e.other:Faction(255,-10,0); -- Gem Choppers, probably to prevent people from spawning a hundred
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 12376})) then -- guessing min faction
		...
		e.other:Faction(288,2,0); -- Merchants of Ak'Anon
		e.other:Faction(333,2,0); -- King Ak'Anon
		e.other:Faction(238,-2,0); -- Dark Reflection
