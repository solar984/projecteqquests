...
function event_timer(e)
	...
	if ( e.timer == "spawn_beast" ) then
    	eq.unique_spawn( 13022, 0, 0, 345.57, 1290, 22.87, 104); -- NPC: A_Tainted_Beast
    	eq.stop_timer("spawn_beast");
  	end
end
...
function event_trade(e)
	...
	elseif ( qglobals["ranger_epic15_pre"] == "6" and item_lib.check_turn_in(e.self, e.trade, {item1 = 62603}) ) then
		...
		e.other:QuestReward(e.self,{itemid = 62603}) -- Polished Blasted Stone
		e.self:Emote("sways in the breeze. It is impossible to tell if he is trying to remember the poem or if he has fallen asleep.");
		...
		e.self:Say("That is the poem. I hope you like it. Now, you shall have to decide how this poem applies to you, or to others, without my help. Return to me when you have what you need and we shall see if we can cleanse that stone.' You had plenty of time to write down the poem as Xanuusus recited it.");
		e.other:QuestReward(e.self,{itemid = 62632}); -- Poem of the Storms
	...
	elseif ( qglobals["ranger_epic15_pre"] == "8" and item_lib.check_turn_in(e.self, e.trade, {item1 = 62604}) ) then
		...
		e.other:QuestReward(e.self,{itemid = 62604}); -- Stone of Winde
