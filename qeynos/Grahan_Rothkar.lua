...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) <= 4 and item_lib.check_turn_in(e.self, e.trade, {item1 = 20031})) then -- requires amiably
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) <= 4 and item_lib.check_turn_in(e.self, e.trade, {item1 = 12188})) then -- requires amiably
		...
		-- Confirmed Live Factions and Experience
		e.other:Faction(311,75); -- Steel Warriors
		e.other:Faction(262,15); -- Guards of Qeynos
		e.other:Faction(230,-11); -- Corrupt Qeynos Guards
		e.other:Faction(330,-11); -- Freeport Militia
		e.other:Faction(281,15); -- Knights of Truth
