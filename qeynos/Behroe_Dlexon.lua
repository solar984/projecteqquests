...
function event_say(e)
	if(e.message:findi("hail")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= -200) then -- verified faction amount
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 50 and item_lib.check_turn_in(e.self, e.trade, {item1 = 18027})) then
		...
		e.other:Faction(284,8,0); -- Faction: League of Antonican Bards
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 50 and item_lib.check_turn_in(e.self, e.trade, {item1 = 18021})) then
