function event_say(e)
	...
	elseif(e.message:findi("other components")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100) then
			...
			e.other:SummonItem(17932);--Empty Chest
		elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0) then
			e.self:Say("When you learn to serve the Dead, then I will find the time to speak of such things.");
		else
			e.self:Say("How dare you enter my presence?!  In the name of the Dead I should strike you down!");		
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13389})) then--Box of Beetle Eyes
		...
		e.other:Faction(275,-1); --Keepers of Art
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13395, item2 = 10124},1,text)) then -- Beetle Eye Chest and Mammoth Tusks
		...
		e.other:Faction(239,20); --The Dead
		e.other:Faction(303,3); --Queen Cristanos Thex
		e.other:Faction(278,-3); --King Naythox Thex
		e.other:Faction(275,-3); --Keepers of Art
		e.other:Faction(245,-3); --Eldritch Collective
