...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {gold = 6}, 0)) then
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {gold = 8}, 0)) then
		e.self:Say("May the breath of life pass through your failing body and cast its foes from you. Unnngh... Praise Innoruuk!");
		e.self:CastSpell(95,e.other:GetID()); -- Counteract Poison
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {gold = 10}, 0)) then
		e.self:Say("May the breath of life pass through your failing body and cast its foes from you. Unnngh... Praise Innoruuk!");
