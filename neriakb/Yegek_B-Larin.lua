function event_say(e)
	...
	elseif(e.message:findi("seloxia")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100) then
			...
			e.other:SummonItem(17942); -- Item: Empty Bag
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13887})) then
		e.self:Say("Very nice work. Here are some provisions. Now are you [ready for the second test]?");
		e.other:Faction(270, 5); --Indigo Brotherhood
		e.other:Faction(326,-1); --Emerald Warriors
		e.other:Faction(311,-1); --Steel Warriors
		e.other:Faction(1522,-10); --Primordial Malice
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13888})) then
		...
		e.other:Faction(270, 15); --Indigo Brotherhood
		e.other:Faction(326,-2); --Emerald Warriors
		e.other:Faction(311,-1); --Steel Warriors
		e.other:Faction(1522,-30); --Primordial Malice
