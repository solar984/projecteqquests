...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 18200})) then		
		...
		e.other:Faction(370, 10);--Dreadguard Inner
		e.other:Faction(334, 1);--Dreadguard Outer
		e.other:Faction(236, 1);--Dark Bargainers
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 19071,item2 = 19070,platinum = 1000},1,text)) then
		e.self:Say("I have done what you asked, now begone from this place, I can bear your stink no longer.");
		e.other:QuestReward(e.self,0,0,0,0,18302,25000);
	end
