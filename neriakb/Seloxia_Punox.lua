...
function event_enter(e)
	if e.other:HasItem(18751) then
		e.other:Message(MT.Yellow,"The cold voice of a female fills your mind with dread, 'I am Seloxia Punox. Seek me out in the Cauldron of Hate and hand me our note so that you may begin on the path of the Warrior. True power can be yours should you have the will to train hard enough!'");
	end
end
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 18751})) then
		...
		e.other:Faction(270,100); --Indigo Brotherhood
		e.other:Faction(326,-15); --Emerald Warriors
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 18843})) then   -- Sealed Letter (Letter To Seloxia)
		...
		e.other:Faction(270, 10); --Indigo Brotherhood
		e.other:Faction(326,-1); --Emerald Warriors
