...
function event_say(e)
	...
	elseif e.message:findi("Korbuk sent me") and qglobals["warrior_epic"] ~= nil and qglobals["warrior_epic"] >= "11" then
	...
	elseif e.message:findi("gemming tool") and qglobals["warrior_epic"] ~= nil and qglobals["warrior_epic"] >= "13" then
	...
	elseif e.message:findi("spell called fire") and qglobals["warrior_epic"] ~= nil and qglobals["warrior_epic"] >= "13" then
...
function event_trade(e)
	...
	local qglobals = eq.get_qglobals(e.other);
	if qglobals["warrior_epic"] ~= nil and qglobals["warrior_epic"] >= "11" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60312, item2 = 60313, item3 = 60314, item4 = 60315}) then --4x Stone of Eternal Power
	...
	if qglobals["warrior_epic"] ~= nil and qglobals["warrior_epic"] >= "12" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60316, item2 = 15433}) then --Gemming Tool and Spell:Fire
	...
	if qglobals["warrior_epic"] ~= nil and qglobals["warrior_epic"] >= "13" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60311}) then --Glistening Hilt
		...
		e.other:QuestReward(e.self,{itemid = 60318});  -- Item: Carved Keg Stamp
	end
	if qglobals["warrior_epic"] ~= nil and qglobals["warrior_epic"] >= "13" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60319}) then --Tube of Setting Solution
		...
		e.other:QuestReward(e.self,{itemid = 60320});  -- Item: Hilt of Eternal Power
	end
