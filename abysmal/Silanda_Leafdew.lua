...
local qglobals = eq.get_qglobals(e.other)
	if e.message:findi("hail") then
		e.self:Say("Hi. It's a wonderful view. I'm not sure that it makes up for what we've [done] to get here, though.");
	elseif qglobals["druid_epic"] ~= nil and qglobals["druid_epic"] == "5" and e.message:findi("stabilizer") then
		e.self:Say("A stabilizer? Well, there are several in nature. What do you need it for?");
	elseif qglobals["druid_epic"] ~= nil and qglobals["druid_epic"] == "5" and e.message:findi("I need to purify a Mind Crystal") then
...
function event_trade(e)
	...
	local qglobals = eq.get_qglobals(e.other)
	if item_lib.check_turn_in(e.self, e.trade, {item1 = 62846}) then --Item: Mind Crystal
	...
	elseif item_lib.check_turn_in(e.self, e.trade, {item1 = 62857}) then --Item: Cleansed Mind Crystal
		...
		e.other:SummonItem(62857); -- Item: Cleansed Mind Crystal
	elseif qglobals["druid_epic"] ~= nil and qglobals["druid_epic"] == "12" and item_lib.check_turn_in(e.self, e.trade, {item1 = 62885}) then -- Item: Synched Leather Bag
		...
		e.other:SummonItem(62875); -- Item: Insulated Container Sketch		
	end
