...
function event_say(e)
	if e.message:findi("hail") then
		e.self:Say("Hello" .. e.other:GetCleanName() .. ". What's that I see? Are you [smirking] at me? That's not very nice of you.");
...
function event_trade(e)
	...
	if qglobals["warrior_epic"] ~= nil and qglobals["warrior_epic"] >= "2" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60295, item2 = 60296, item3 = 60297, item4 = 60298}) then -- Items: Decomposing Page 1,2,3 and Decomposing Cover
		...
		e.other:SummonItem(60299); -- Item: Recounted History of War
	end
