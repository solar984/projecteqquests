function event_say(e)
	...
	elseif(e.message:findi("keven")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100) then -- not verified as exact
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13768})) then	--White Wolf Skin
		...
		e.other:Faction(244, -1);     							-- Ebon Mask
		e.other:QuestReward(e.self,0,0,0,0,7365,250); --Field Priest's Bracer
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 16486, item2 = 16486, item3 = 16486, item4 = 2125})) then 					--Silk Thread x 3, Large Tattered Skullcap
		...
		e.other:Faction(327, 1);      							-- Shamen of Justice
		e.other:Faction(328, 1);      							-- Merchants of Halas
		e.other:Faction(223, -1);      							-- Circle Of Unseen Hands
		e.other:Faction(336, -1);       							-- Coalition of Tradefolk Underground
		e.other:Faction(244, -1);     							-- Ebon Mask
		e.other:QuestReward(e.self,0,0,0,0,7366,400); --Field Priest's Cap
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13897, item2 = 13897, item3 = 13897, item4 = 13897})) then	--Goblin Ice necklacess x 4
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 13915, item2 = 7005, item3 = 8011})) then 									--Gnoll Fang, Giant Snake Fang, Hunting Bow
	...
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 12223})) then 									--Wrath Orc Wristbands
		...
		e.other:Faction(327, 50);      							-- Shamen of Justice
		e.other:Faction(328, 7);      							-- Merchants of Halas
		e.other:Faction(223, -7);      							-- Circle Of Unseen Hands
		e.other:Faction(336, -7);       							-- Coalition of Tradefolk Underground
