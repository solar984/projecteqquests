function event_say(e)
	...
	elseif(e.message:findi("member of the white rose")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0) then -- needs indifferent.
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 5243, item2 = 5243, item3 = 5243, item4 = 5243})) then	-- requires indifferent, Mammoth Calf Hide X 4
		...
		e.other:Faction(305,5,0);	-- Faction Rogue's of the White Rose
