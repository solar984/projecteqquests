...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 1330})) then	-- requires indifferent, Patched Gnoll Fur Bundle
		e.self:Say("You have done well. Here is a small reward for your effort.");
		-- Confimred Live Factions
		e.other:Faction(305,5);	-- Faction Rogue's of the White Rose
		e.other:QuestReward(e.self,0,math.random(10),math.random(10),math.random(0,1),1349,3000);	--Fang of the Wolf
	elseif(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 0 and item_lib.check_turn_in(e.self, e.trade, {item1 = 7507})) then	-- requires indifferent, Shrieking Ahlspiess
