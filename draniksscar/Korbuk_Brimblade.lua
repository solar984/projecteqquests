...
-- items: 60300, 60285, 60286, 60288, 60292, 60293, 60295, 60296, 60297, 60298, 60299, 60301, 60302, 60303, 60304, 60305, 60306, 60307, 60311, 60312, 60313, 60314, 60315, 60320, 60322, 60325, 60326, 47100, 60321, 60327, 60329
function event_say(e)
	...
	if(e.other:Class() == "Warrior") then
		...
		elseif(e.message:findi("work")) then
			if ((e.other:HasItem(10910) and e.other:HasItem(10909)) or (e.other:HasItem(66176) and e.other:HasItem(66177)) or e.other:HasItem(10908) or e.other:HasItem(66175) or qglobals["warrior_pre"] == "6") then --Have 1.0 or have finished prequest
...
function event_trade(e)
	...
	local qglobals = eq.get_qglobals(e.other);
	if(qglobals["warrior_pre"] == "1" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60285})) then --hand in Korbuk's Weapon Plans (dropped by The Diaku Overseer in PoTactics)
		...
		e.other:SummonItem(60285); --get Korbuk's Weapon Plans back
	end
	if(qglobals["warrior_pre"] == "2" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60286})) then --hand in Elegant Shank (dropped by Shoqui the Forgotten in Hate's Fury)
	...
	if(qglobals["warrior_pre"] == "3" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60288})) then --hand in Emblazoned Champion's Hilt (received from Kargek Redblade from previous step)
	...
	if(qglobals["warrior_pre"] == "5" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60292})) then --hand in Dardek's Forged Blade (Received from Dardek Bladewright after previous step)
  ...
  if (qglobals["warrior_epic"] ~= nil) then
    if(qglobals["warrior_epic"] >= "1" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60295, item2 = 60296, item3 = 60297, item4 = 60298})) then --Decomposing Page 1,2,3 and Decomposing Cover (all groundspawns)
    ...
    if(qglobals["warrior_epic"] >= "2" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60299})) then --Recounted History of War (get from previous step)
    ...
    if(qglobals["warrior_epic"] >= "2" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60301})) then --Blackfall Ore (From previous step)
    ...
    if(qglobals["warrior_epic"] >= "2" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60303})) then --Blackfall Blade (from previous step)
    ...
    if(qglobals["warrior_epic"] >= "3" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60304})) then --Vial of Blackfall Blood (from Vxed encounter)
    ...
    if(qglobals["warrior_epic"] >= "4" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60306})) then --Potion of Blackfall Spirit (from Kimber in EF)
    ...
    if(qglobals["warrior_epic"] >= "5" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60307})) then --Decrepit Hilt (from Girplan Devourer in Bloodfields)
    ...
    if(qglobals["warrior_epic"] >= "9" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60311})) then --Glistening Hilt (from Corfia)
    ...
    if(qglobals["warrior_epic"] == "10" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60312, item2 = 60313, item3 = 60314, item4 = 60315})) then --4 diff Stone of Eternal Power
    ...
    if(qglobals["warrior_epic"] >= "13" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60320})) then --Hilt of Eternal Power
    ...
    if(qglobals["warrior_epic"] >= "16" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60322})) then --Kreljnok's Muddled Rage from Possessed Brute in MPG
    ...
    if(qglobals["warrior_epic"] >= "18" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60325})) then --Larnik's Locked Purification Kit, dropped by Larnik the Recluse in EW
    ...
    if(qglobals["warrior_epic"] >= "18" and item_lib.check_turn_in(e.self, e.trade, {item1 = 47100})) then --Globe of Discordant Energy
    ...
    if(qglobals["warrior_epic"] >= "19" and item_lib.check_turn_in(e.self, e.trade, {item1 = 60321, item2 = 60327})) then --Kreljnok's Focused Rage and Champion's Sword of Eternal Power
