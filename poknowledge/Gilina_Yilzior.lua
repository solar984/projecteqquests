...
function event_trade(e)
	...
	if(qglobals["shadowknight_epic"] == "5" and item_lib.check_turn_in(e.self, e.trade, {item1 = 11407})) then --hand in first version of Sheathed Innoruuk's Voice
	...
	if(qglobals["shadowknight_epic"] == "6" and item_lib.check_turn_in(e.self, e.trade, {item1 = 20899, item2 = 16813, item3  = 16793, item4 = 20079})) then --hand in Silithis's Head (drops from Silithis Sisnta in Timorous Deep), Gubblegrot's Head (drops from Gubblegrot Smashfist in OoT), Skurza's Head (drops from Skurza Slicekutt in Swamp of No Hope), and Giligatabbus's Head (drops from Giligatabbus Igglebix in Iceclad Ocean)
	...
	if(qglobals["shadowknight_epic"] == "7" and item_lib.check_turn_in(e.self, e.trade, {item1 = 20904, item2 = 20904, item3  = 20904, item4 = 22199})) then --hand in 3x Paladin Blood Sample (from Paladins in Kithicor Forest Event) and 2nd version of Sheathed Innoruuk's Voice (Received after last step)
	...
	if(qglobals["shadowknight_epic"] == "8" and item_lib.check_turn_in(e.self, e.trade, {item1 = 55901})) then --hand in Sir Elmonious's Holy Symbol (drops from Sir Elmonious Falmont in Western Wastes)
	...
	if(qglobals["shadowknight_epic"] == "9" and item_lib.check_turn_in(e.self, e.trade, {item1 = 19098})) then --hand in Kilidna's Head (drops from Kilidna in City of Mist)
	...
	if(qglobals["shadowknight_epic"] == "10" and item_lib.check_turn_in(e.self, e.trade, {item1 = 24612, item2 = 23489, item3 = 24632})) then --hand in Lhranc's Corroded Bracer (dropped from golems in Charasis), Lhranc's Pendant (dropped from black reavers in City of Mist), Lhranc's Ring (dropped from various undead in Kithicor)
