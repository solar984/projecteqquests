...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 13118})) then
		...
		e.other:Faction(266,2); -- High Council of Erudin
		e.other:Faction(265,-3); -- Heretics 
		e.other:Faction(267,3); -- High Guard of Erudin
		e.other:QuestReward(e.self,0,0,0,0,13122,300); -- Item: Eruds Tonic -- verified exp on live
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 13123})) then
		...
		-- Confirmed Live Faction
		e.other:Faction(231,30); -- Craftkeepers
		e.other:Faction(266,3); -- High Council of Erudin
		e.other:Faction(265,-4); -- Heretics
		e.other:Faction(267,4); -- High Guard of Erudin
