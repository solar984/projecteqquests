function event_say(e)
	...
	elseif(e.message:findi("all is not bright above the clouds") and not eq.is_content_flag_enabled("Classic_OldWorldDrops")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= 500) then
			...
			e.other:SummonItem(13108); -- Item: Tearons Bracer
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 12267,item2 = 13109})) then
		...
		e.other:Faction(5002,18); -- Faction: Anti-mage
		e.other:QuestReward(e.self,{itemid = 9312, exp = 2000}); -- Item: Silent Watch Shield
		eq.depop_with_timer();
	end
