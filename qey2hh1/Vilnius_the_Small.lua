...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("" .. e.other:GetCleanName() .. ", just because I am short don't mean I can't bounce your " .. e.other:Race() .. " rump all over these hills. Now, do you have something for me, or are you just in search of a rump bouncing?");
	...
	elseif(e.message:findi("stanos")) then
		e.self:Say("If you need to ask that question, you do not need to know the answer. At least not from me.");
	elseif(e.message:findi("who.* you")) then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 28014})) then -- Stanos' Pouch
		...
		e.other:QuestReward(e.self,0,0,0,0,0,1500);
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 5411,item2 = 5410,item3 = 5401,item4 = 5308},1,text)) then -- Fleshripper, Painbringer, Mithril Two-Handed Sword, Gigantic Zweihander
