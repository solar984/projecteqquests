...
function event_say(e)
	if(e.message:findi("hail")) then
		if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= -100) then --guessing on faction level, only tested at scowls
...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= -100 and item_lib.check_turn_in(e.self, e.trade, {gold = 5})) then
		...
		-- confirmed live, no factions, no xp
	end
