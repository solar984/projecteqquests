...
function event_trade(e)
	...
	if(e.other:GetModCharacterFactionLevel(e.self:GetPrimaryFaction()) >= -100 and item_lib.check_turn_in(e.self, e.trade, {item1 = 28014})) then
		e.self:Say("Ah, we have been expecting this. Let me get Stanos, he will want to inspect it first, but here are your coins.");
		e.other:Faction(332,50); -- Faction: Highpass Guards
		e.other:Faction(329,7); -- Faction: Carson McCabe
		e.other:Faction(331,7); -- Faction: Merchants of Highpass
		e.other:Faction(230,2); -- Faction: Corrupt Qeynos Guards
		e.other:Faction(330,2); -- Faction: The Freeport Militia
		e.other:QuestReward(e.self,{gold = 100, platinum = 25, exp = 10000});
		eq.unique_spawn(5088,0,0,336,10,45,225); -- NPC: Stanos_Herkanor
	end
